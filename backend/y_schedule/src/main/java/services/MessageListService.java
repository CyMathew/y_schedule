...
public class MessageListService {
	...
	public JSONArray buildJsonList(JSONObject json, Integer id) {
		...
		for (MessageListBean m : list) {
			if (m.getUser1() == id) {
				messageList.put(
						new JSONObject().put("messageListID", (m.getMessage_list_id())).put("user", (m.getUser2())));
			} else {
				messageList.put(
						new JSONObject().put("messageListID", (m.getMessage_list_id())).put("user", (m.getUser1())));
			}
		}
